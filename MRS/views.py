...
def generateRecommendation(request):
    ...
    #Movie Data Frames
    for item in movie:
        x=[item.id,item.title,item.movieduration,item.image.url,item.genres,item.imdbrating] 
        y+=[x]
    movies_df = pd.DataFrame(y,columns=['movieId','title','movieduration','image','genres','imdbrating'])
